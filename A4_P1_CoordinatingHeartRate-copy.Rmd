...
#####
#LOOP/FUNKTION TANKER
#Der skal nok bygges 3/4 funktioner
#1. til preprocessing:
#   - Downsampling
#   - Rescaling
#   - Removing outliers
#Alt skal gemmes til en dataframe som vi kan arbejde videre med.
#
#2. til plots
#Funktion der laver grid.arrange plottet for alle grupper. 
#Output skal enten være at den printer/gemmer dem allesammen som filer så vi selv kan scrolle igennem, ellers skal
#de gemmes (til en liste/df??) på en eller anden sindrig måde..
#
#3. til at finde optimal parameters
#Tror pretty much vi kan genbruge den vi tidligere har lavet
#
#4. til at køre crqa
#Igen, med lidt held kan vi nærmest genbruge den gamle funktion
#
#Generelt er der meget hjælp at hente i A3_P1!
#####
preproz = function(filename){ #requires the removeOuts function and a set threshold
  ...
  if (length(opt_param) > 2) {
    df$optRadiusHR = opt_paramHR[1]
    df$optEmbdimHR = opt_paramHR[2]
    df$optDelayHR = opt_paramHR[3]
    } else {
    df$optRadiusHR = NA
    df$optEmbdimHR = NA
    df$optDelayHR = NA
  ...
  #if the optimize function succeeded, save the results to the dataframe - if not, put NAs 
  if (length(opt_param) > 2) {
    df$optRadiusResp = opt_paramHR[1]
    df$optEmbdimResp = opt_paramHR[2]
    df$optDelayResp = opt_paramHR[3]
    } else {
    df$optRadiusResp = NA
    df$optEmbdimResp = NA
    df$optDelayResp = NA
